...
def pvpg_gt(atl03path, atl08path, gt):
    """
    Variation of pvpg function that focuses on a single groundtrack. The purpose of this
    function is to compare a single beam in different files. This is a quick way to reduce
    the quantity of output (1 track vs 6 tracks shown) and retaining diversity of output.
    Good for experimenting with methods and checking for a degree of generality.

    atl03path - Path/to/ATL03/file
    atl08path - Path/to/matching/ATL08/file
    gt - One of "gt1l", "gt1r", "gt2l", "gt2r", "gt3l", or "gt3r", indiicating the chosen ground track to focus on.
    """
...
def pvpg_gt_penalized(atl03path, atl08path, gt, f_scale = .1, loss = 'arctan', model = model, residuals = residuals):
    """
    Experimentation function focused on single groundtracks to look at various regression
    methods. In particular, the focus is on outlier robust regression.
    ...
    result = least_squares(residuals, initial_guess, loss=loss, f_scale=f_scale, args=(X, Y))
