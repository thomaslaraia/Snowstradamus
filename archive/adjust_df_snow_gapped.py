...
def parse_args():
    ...
    parser.add_argument('--input_pickle', type=str, default='five_sites_data_snow_cc', help='Name of input pickle file (without extension)')
...
def find_matching_hdf(year, day_of_year, start, hdf_dirf):
    ...
    prefix = f'{start}10A1.A{year}{day_of_year}'
    suffix = '.hdf'
...
def hdf_to_sinusoidal(hdf_path):
    ...
    dataset = hdf.select('NDSI_Snow_Cover')
    data = dataset[:]
...
for index, row in df.iterrows():
    ...
    for start in ['MOD','MYD']:
        ...
        hdf_dir = f'../data_store/data/{start}10A1_{place}/'
